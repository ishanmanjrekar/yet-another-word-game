...
- **Algorithm:** Pass the unselected grid array through a Fisher-Yates array shuffle algorithm and trigger an animation state update in the UI.
...
- **Algorithm:**
  ...
  4. Apply a `.pulse-highlight` CSS class (or equivalent animation wrapper) to that specific tile in the UI.
...
- **Algorithm:**
  ...
  4. Select the matching tile index and map it directly into the target slot, triggering the standard selection UI animation.
